...
async function verifyStoredUpload(stored, expected) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      const saved = await readBuffer(stored.key);
      ...
      lastError = error;
      if (attempt < 2) await sleep(160 * (attempt + 1));
    }
  }
...
async function processFullSync(rawLeagueId, limit = 8) {
  const id = leagueId(rawLeagueId);
  ...
  const batchLimit = Math.max(1, Math.min(Number(limit || 8), 16));
