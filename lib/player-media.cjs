...
async function buildDirectState(rawLeagueId, options = {}) {
  ...
  if (!fresh && cached && cached.expiresAt > Date.now()) return clone(cached.state);
  ...
  directStateCache.set(id, { expiresAt: Date.now() + DIRECT_CACHE_TTL_MS, state: clone(state) });
  return clone(state);
}
...
async function directSearchProvider(rawLeagueId, query, teamName = "") {
  const state = await buildDirectState(rawLeagueId);
  return Object.values(state.catalog.players || {})
    .map((candidate) => ({
      candidate,
      score: queryScore(query, candidate) + (teamName && String(candidate.teamKey) === clubKey(teamName) ? 30 : 0)
    }))
...
function manualDirectLinkDisabled() {
  throw new Error("I collegamenti manuali non vengono salvati: le facce usano URL BSD diretti senza scritture Blob.");
}
...
module.exports = {
  ...
  isResolvedEntry,
  keepExistingOrUnresolved,
  linkManual: manualDirectLinkDisabled,
  linkTeamManual: manualDirectLinkDisabled,
