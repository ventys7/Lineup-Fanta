...
function clubKey(v) {
  const ALIASES = { "man utd": "manchester united", "man united": "manchester united", "barca": "barcelona" };
...
async function buildDirectState(rawId, options = {}) {
  ...
  if (!options.fresh && databaseConfigured()) {
    try {
      ...
      if (cached && cached.state && cached.state.players && (new Date(cached.generatedAt).getTime() + CACHE_TTL) > Date.now()) {
        return cached.state;
      }
    } catch (e) {}
  }
  ...
  for (const club of uniqueClubs) {
    try {
      ...
      const bsdRoster = (bsdData?.results || []).map(p => ({ id: String(p.id), name: p.full_name || p.name }));
      assets.filter(a => a.realTeam === club).forEach(asset => {
        ...
        if (ovr) {
          manifest.players[key] = { status: "resolved", photoUrl: `https://sports.bzzoiro.com/img/player/${ovr.id}/`, externalId: ovr.id };
        } else {
          const best = bsdRoster.find(c => normalize(c.name) === normalize(asset.displayName));
          if (best) {
            manifest.players[key] = { status: "resolved", photoUrl: `https://sports.bzzoiro.com/img/player/${best.id}/`, externalId: best.id };
          } else {
            manifest.players[key] = { status: "unresolved", listoneName: asset.displayName, realTeam: asset.realTeam, candidates: bsdRoster.slice(0, 5) };
          }
        }
      });
    } catch (e) {}
  }
  if (databaseConfigured() && Object.keys(manifest.players).length > 0) {
    await writeManifestCache(id, manifest);
  }
  return manifest;
}
