...
function validCachedState(value) {
  return Boolean(value?.manifest?.players && value?.catalog?.players && Array.isArray(value?.teamIssues));
}
...
async function buildDirectState(rawLeagueId, options = {}) {
  ...
  if (!fresh && databaseConfigured()) {
    ...
    if (persisted && cacheExpiresAt(persisted.generatedAt) > Date.now() && validCachedState(persisted.state)) {
      ...
      directStateCache.set(id, { expiresAt: cacheExpiresAt(persisted.generatedAt), state: clone(state) });
      return state;
    }
  }
  ...
  const expiresAt = Date.now() + DIRECT_CACHE_TTL_MS;
  directStateCache.set(id, { expiresAt, state: clone(state) });
