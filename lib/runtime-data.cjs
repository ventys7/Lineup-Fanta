...
function findBlobEnvironmentValue(exactName, suffix) {
  const exactValue = String(process.env[exactName] || "").trim();
  if (exactValue) {
    return { name: exactName, value: exactValue };
  }
...
function resolveBlobCredentials() {
  const token = findBlobEnvironmentValue(
    "BLOB_READ_WRITE_TOKEN",
    "BLOB_READ_WRITE_TOKEN"
  );
  const storeId = findBlobEnvironmentValue(
    "BLOB_STORE_ID",
    "BLOB_STORE_ID"
  );
  ...
  return {
    token: token?.value || "",
    tokenEnvironmentName: token?.name || null,
    storeId: storeId?.value || "",
    storeIdEnvironmentName: storeId?.name || null,
    oidcToken
  };
}
...
function blobAuthOptions() {
  ...
  if (credentials.storeId && credentials.oidcToken) {
    return {
...
function hasBlobCredentials() {
  return Object.keys(blobAuthOptions()).length > 0;
}
...
async function deleteManagedLogo(value) {
  ...
  await del(value, blobAuthOptions());
}
