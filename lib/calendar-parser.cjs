const { extractSeason } = require("./season.cjs");
...
function parsePublishedCalendar(html, expectedMatches = 4) {
  ...
  const documentSeason = extractSeason(...lines);
  const matchdays = [];
  let current = null;

  for (const line of lines) {
    const heading = line.match(/^([IVXLCDM]+)\s+GIORNATA(?:\s+(\d{2,4}\s*[-/]\s*\d{2,4}))?\s*\((\d+)\)(?:\s+(\d{2,4}\s*[-/]\s*\d{2,4}))?$/i);
    if (heading) {
      if (current) matchdays.push(current);
      current = {
        season: extractSeason(heading[2], heading[4], line) || documentSeason,
        realRoundNumber: romanToInteger(heading[1]),
        fantasyMatchdayNumber: Number(heading[3]),
        matches: []
      };
      continue;
    }
